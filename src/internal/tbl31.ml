...
let crc_tbl =
  Array.init 128 (fun i ->
      let rec loop j sum =
        if j < 0 then sum
        else if i land (1 lsl j) <> 0 then loop (j - 1) (sum lxor (poly lsr j))
        else loop (j - 1) sum
      in
      loop (7 - 1) 0)
...
let ( lsl ) x n =
  if n >= Sys.word_size then 0
  else if n <= ~-Sys.word_size then 0
  else if n < 0 then x lsr ~-n
  else x lsl n
...
module MakeNode (Sub : NodeType) = struct
  ...
  module NodeHash = struct
    ...
    let equal x y =
      ...
      let b = untag y in
      let rec loop i =
        if i < 0 then true else if a.(i) == b.(i) then loop (i - 1) else false
      in
  ...
  let hashcons a =
    ...
    let x = Tag (b, n) in
    try NodePool.find pool x
    with Not_found ->
      NodePool.add pool x;
      x

  let make_raw def = Array.make (if level = 3 then 128 else 256) (Sub.make def)
  ...
  let of_set n0 def s v =
    ...
    for i = 0 to if level = 3 then 127 else 255 do
      ...
      if ISet.is_empty s' then () else a.(i) <- Sub.of_set n1 def s' v
    done;
    hashcons a
end
...
module ArrayLeaf (H : Hashtbl.HashedType) = struct
  ...
  module NodeHash = struct
    ...
    let equal x y =
      ...
      let rec loop i =
        if i >= 255 then true
        else if H.equal a.(i) b.(i) then loop (i + 1)
        else false
      in
      loop 0
...
module StringContentsHash = struct
  ...
  let equal x1 x2 =
    let s1 = untag x1 in
    let s2 = untag x2 in
    if Bytes.length s1 <> Bytes.length s2 then false
    else (
      let rec loop i =
        if i < 0 then true
        else if Bytes.get s1 i <> Bytes.get s2 i then false
        else loop (i - 1)
      in
...
module BitsContentsHash = struct
  ...
  let equal x1 x2 =
    ...
    let rec loop i =
      if i < 0 then true
      else if Bitsvect.get a1 i = Bitsvect.get a2 i then loop (i - 1)
      else false
    in
    loop 255
...
module BytesContentsHash = struct
  ...
  let equal x1 x2 =
    ...
    let rec loop i =
      if i < 0 then true
      else if Bytesvect.get a1 i = Bytesvect.get a2 i then loop (i - 1)
      else false
    in
    loop 255
